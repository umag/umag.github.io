...
All of that exploration ran through swamp. The whole integration is one extension model with a handful of methods — `authorize`, `exchange`, `probe`, `sync`, `derive` — and every answer the API gave me landed as versioned, queryable data. `probe` takes a list of candidate type ids, pulls a small sample of each, and stores the first DataPoint as a `raw-<type>` resource; the status map above came straight out of those runs. `swamp data get google-health raw-exercise` then shows the exact shape the API really returns, nested keys, string numbers and all. Once a shape was confirmed, I wired it into `sync`, which maps it to Influx line protocol and streams it into VictoriaMetrics; each run also records a per-type breakdown of points fetched and mapped, so a silent regression shows up as a number. The exploration itself left an audit trail: I can still query the raw sample that convinced me the numbers arrive as strings.
